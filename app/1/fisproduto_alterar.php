...
if (isset($LOG_CAMINHO)) {
    ...
    $identificacao = date("dmYHis") . "-PID" . getmypid() . "-" . "fisproduto_alterar";
    if (isset($LOG_NIVEL)) {
        if ($LOG_NIVEL >= 1) {
...
if (isset($jsonEntrada['idProduto'])) {
    ...
    $sql = "UPDATE fisproduto SET codigoProduto=$codigoProduto, nomeProduto=$nomeProduto, quantidade=$quantidade, unidCom=$unidCom, valorUnidade=$valorUnidade, valorTotal=$valorTotal, cfop=$cfop, ncm=$ncm, cest=$cest, chaveNFe=$chaveNFe WHERE idProduto = $idProduto";
